...
func (subscribePacket *SubscribePacket) Unpack() {
	...
	subscribePacket.MessageID = buffer[packetShift : packetShift+2]
	packetShift += 2
	topicLength = int(binary.BigEndian.Uint16(buffer[packetShift : packetShift+2]))
	packetShift += 2
	subscribePacket.topic = string(buffer[packetShift : packetShift+topicLength])
	packetShift += topicLength
	subscribePacket.QoS = buffer[packetShift]
